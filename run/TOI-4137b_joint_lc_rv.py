...
# Models
# ------------------------------------------------------------
def batman_flux(params_vec, time_axis):
    t0_bjd, per, rp_over_rstar, a_over_rstar, inc_deg, _, _ = params_vec
...
def rv_model(params_vec, time_axis):
    t0_bjd, per, rp_over_rstar, a_over_rstar, inc_deg, vsini, lambda_deg = params_vec
    ...
    rv_call = ellc.rv(
        ...
        incl=inc_deg,
        a=a_fixed,
        f_c=np.sqrt(ecc) * np.cos(np.deg2rad(-10)),
        f_s=np.sqrt(ecc) * np.sin(np.deg2rad(-10)),
        q=q_fixed,
        shape_1="sphere",
        shape_2="sphere",
        vsini_1=vsini,
    ...
    if isinstance(rv_call, (tuple, list)):
        return np.asarray(rv_call[0])
    return np.asarray(rv_call)


def loglikelihood_joint(p):
    t0_bjd, per, rp_over_rstar, a_over_rstar, inc_deg, vsini, lambda_deg = p
    ...
    lc_model = batman_flux(p, lc_time)
    rv_model_vals = rv_model(p, rv_time)
...
wid = [0.01, 0.001, 0.001, 0.02, 0.02, 0.2, 1.0]
...
parinfo = [
    ...
    {"fixed": False, "limits": [0.0, 30.0], "limited": [True, True]},
    {"fixed": False, "limits": [-90.0, 90.0], "limited": [True, True]},
]
...
best_lc = batman_flux(med, lc_time)
best_rv = rv_model(med, rv_time)

lc_phase = ((lc_time - (med[0] - 2457000.0)) / med[1] + 0.5) % 1.0 - 0.5
rv_phase = ((rv_time - med[0]) / med[1] + 0.5) % 1.0 - 0.5
...
for j, idx in enumerate(sel_idx):
    lc_models[j, :] = batman_flux(all_samples[idx, :], lc_time)

lc_median = np.median(lc_models, axis=0)
lc_p16 = np.percentile(lc_models, 16.0, axis=0)
lc_p84 = np.percentile(lc_models, 84.0, axis=0)
lc_p025 = np.percentile(lc_models, 2.5, axis=0)
lc_p975 = np.percentile(lc_models, 97.5, axis=0)

# RV posterior models
ntime = len(rv_time)
models = np.zeros((nsamp, ntime))
for j, idx in enumerate(sel_idx):
    models[j, :] = rv_model(all_samples[idx, :], rv_time)
